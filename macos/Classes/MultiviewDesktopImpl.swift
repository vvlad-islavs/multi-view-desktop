...
class MultiviewDesktopImpl: NSObject, NSWindowDelegate {
  ...
  func windowShouldClose(_ sender: NSWindow) -> Bool {
    guard let viewId = viewIdForWindow(sender) else { return true }
  ...
  func windowShouldZoom(_ window: NSWindow, toFrame newFrame: NSRect) -> Bool {
    guard let viewId = viewIdForWindow(window) else { return true }
  ...
  private func handleViewMethod(
    ...
    case "setResizable":
      ...
      if v { window.styleMask.insert(.resizable) } else { window.styleMask.remove(.resizable) }
      result(nil)
    ...
    case "setMinimizable":
      ...
      if v { window.styleMask.insert(.miniaturizable) } else { window.styleMask.remove(.miniaturizable) }
      result(nil)
    ...
    case "setClosable":
      ...
      if v { window.styleMask.insert(.closable) } else { window.styleMask.remove(.closable) }
      result(nil)
    ...
    case "setVisibleOnAllWorkspaces":
      ...
      if visible {
        ...
        if onFullScreen { window.collectionBehavior.insert(.fullScreenAuxiliary) }
      } else {
